...
void *PeGetProcAddressA(void *Base, LPCSTR Name)
{
	...
	if (Ret)
	{
		const DWORD_PTR ExpStart = NT->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress + reinterpret_cast<DWORD_PTR>(Base);
		const DWORD_PTR ExpSize = NT->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size;
		const DWORD_PTR RetAddress = reinterpret_cast<DWORD_PTR>(Ret);
		if (RetAddress >= ExpStart && RetAddress < ExpStart + ExpSize)
		{
			// Forwarder
			return 0;
		}
...
int RunHost()
{
	...
	BOOL ok = CreateProcessA("ProcessHost.exe", NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
	...
	{
		DebugLog("Process start failed, error=%lu\n", GetLastError());
	}
