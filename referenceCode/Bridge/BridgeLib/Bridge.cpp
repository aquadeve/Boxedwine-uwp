...
#if defined(_M_ARM)
#include "include/kernel/uapi/asm-arm/asm/unistd.h"
#elif defined(_M_X64)
#include "include/kernel/uapi/asm-x86/asm/unistd_64.h"
#else
#include "include/kernel/uapi/asm-x86/asm/unistd_32.h"
#endif
}
...
void* __data_start = reinterpret_cast<void*>(static_cast<uintptr_t>(0x20000000u));
void* data_start = reinterpret_cast<void*>(static_cast<uintptr_t>(0x20000000u));
#if defined(_M_X64)
const char* __progname = "app_process64";
#else
const char* __progname = "app_process32";
#endif
void* __google_potentially_blocking_region_begin = reinterpret_cast<void*>(static_cast<uintptr_t>(0x80000000u));
void* __google_potentially_blocking_region_end = reinterpret_cast<void*>(static_cast<uintptr_t>(0xFFFFFFFFu));
...
long _bridge_syscall(long __number, ...)
{
	...
	if (__number == __NR_clone)
	{
		...
		ret = sys_clone_imp(NULL, flags, child_stack, ptid, ctid);
#else
...
{
	int argc = 0;
	arg_num_length(argc, argv);

	int envc = 0;
	arg_num_length(envc, envp);

	return bridge_execve(filename, argc, argv, envc, envp);
}
